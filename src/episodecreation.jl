...
function yrage_episodes(node)
    ...
    while hstate !== nothing
        ...
        # open(joinpath(basedirectory, node, "Episodes", "DebugTmp$(i).arrow"),"w"; lock = true) do io
        ...
        # open(joinpath(basedirectory, node, "Episodes", "DebugEpisodes1Tmp$(i).arrow"),"w"; lock = true) do io
        #     Arrow.write(io, t, compress=:zstd)
        # end
        # End Debug
        ...
        # Debug
        # t = filter(row -> row.IndividualId == 11100 || row.IndividualId == 17829, e)
        # open(joinpath(basedirectory, node, "Episodes", "DebugEpisodes2Tmp$(i).arrow"),"w"; lock = true) do io
        #     Arrow.write(io, t, compress=:zstd)
        # end
        # End Debug
        ...
        # open(joinpath(basedirectory, node, "Episodes", "DebugEpisodes3Tmp$(i).arrow"),"w"; lock = true) do io
        #     Arrow.write(io, t, compress=:zstd)
        # end
        # End Debug
        ...
        e = transform(groupby(sort(episodes,[:IndividualId, :StartDate]), [:IndividualId]), :IndividualId => eachindex => :Episode, nrow => :Episodes)
        open(joinpath(basedirectory, node, "Episodes", "SurveillanceEpisodesYrAge$(i).arrow"),"w"; lock = true) do io
            Arrow.write(io, e, compress=:zstd)
        end
        hstate = iterate(residentdaybatches, hst)
        @info "Node $(node) batch $(i) completed with $(nrow(e)) episodes after $(round(now()-t, Dates.Second))"
    ...
    combinebatches(basedirectory,node,"Episodes","SurveillanceEpisodesYrAge", i-1)
    return nothing
end
"Do yr-age episodes QA"
function yrage_episodeQA(node)
    basedirectory = settings.BaseDirectory
    df = Arrow.Table(joinpath(basedirectory, node, "Episodes", "SurveillanceEpisodesYrAge_batched.arrow")) |> DataFrame
    # StartFlags
    ...
    sort!(sf)
    XLSX.writetable(joinpath(basedirectory, node, "Episodes","QC", "EpisodesYrAgeQA.xlsx"), collect(eachcol(sf)), names(sf), overwrite=true, sheetname="StartFlags")
    # EndFlags
    ...
    transform!(sf, names(sf, Int16) .=> ByRow(Int), renamecols=false) #needed by XLSX
    sort!(sf)
    addsheet!(joinpath(basedirectory, node, "Episodes","QC", "EpisodesYrAgeQA.xlsx"), sf, "EndFlags")
    # Episode1StartFlags
    sf = filter(r -> r.Episode == 1, df) #Start episodes only
    ...
    transform!(sf, names(sf, Int16) .=> ByRow(Int), renamecols=false) #needed by XLSX
    sort!(sf)
    addsheet!(joinpath(basedirectory, node, "Episodes","QC", "EpisodesYrAgeQA.xlsx"), sf, "Episode1StartFlags")
    # LastEpisodeEndFlags
    sf = filter(r -> r.Episode == r.Episodes, df) #Last episodes only
    ...
    transform!(sf, names(sf, Int16) .=> ByRow(Int), renamecols=false) #needed by XLSX
    sort!(sf)
    addsheet!(joinpath(basedirectory, node, "Episodes","QC", "EpisodesYrAgeQA.xlsx"), sf, "LastEpisodeEndFlags")
    # Births per year
    sf = filter(r -> r.Born == 1, df)
    sf = combine(groupby(sf, [:CalendarYear]), nrow => :n)
    transform!(sf, names(sf, Int8) .=> ByRow(Int), renamecols=false) #needed by XLSX
    sort!(sf)
    addsheet!(joinpath(basedirectory, node, "Episodes", "QC", "EpisodesYrAgeQA.xlsx"), sf, "Births")
    # Enumerations per year
    sf = filter(r -> r.Enumeration == 1, df)
    sf = combine(groupby(sf, [:CalendarYear]), nrow => :n)
    transform!(sf, names(sf, Int8) .=> ByRow(Int), renamecols=false) #needed by XLSX
    sort!(sf)
    addsheet!(joinpath(basedirectory, node, "Episodes", "QC", "EpisodesYrAgeQA.xlsx"), sf, "Enumerations")
    # InMigrations per year
    sf = filter(r -> r.InMigration == 1, df)
    sf = combine(groupby(sf, [:CalendarYear]), nrow => :n)
    transform!(sf, names(sf, Int8) .=> ByRow(Int), renamecols=false) #needed by XLSX
    sort!(sf)
    addsheet!(joinpath(basedirectory, node, "Episodes", "QC", "EpisodesYrAgeQA.xlsx"), sf, "InMigrations")
    # LocationEntry per year
    sf = filter(r -> r.LocationEntry == 1, df)
    sf = combine(groupby(sf, [:CalendarYear]), nrow => :n)
    transform!(sf, names(sf, Int8) .=> ByRow(Int), renamecols=false) #needed by XLSX
    sort!(sf)
    addsheet!(joinpath(basedirectory, node, "Episodes", "QC", "EpisodesYrAgeQA.xlsx"), sf, "LocationEntries")
    # ExtResStart per year
    sf = filter(r -> r.ExtResStart == 1, df)
    sf = combine(groupby(sf, [:CalendarYear]), nrow => :n)
    transform!(sf, names(sf, Int8) .=> ByRow(Int), renamecols=false) #needed by XLSX
    sort!(sf)
    addsheet!(joinpath(basedirectory, node, "Episodes", "QC", "EpisodesYrAgeQA.xlsx"), sf, "ExtResStarts")
    # Deaths per year
    sf = filter(r -> r.Died == 1, df)
    sf = combine(groupby(sf, [:CalendarYear]), nrow => :n)
    transform!(sf, names(sf, Int8) .=> ByRow(Int), renamecols=false) #needed by XLSX
    sort!(sf)
    addsheet!(joinpath(basedirectory, node, "Episodes", "QC", "EpisodesYrAgeQA.xlsx"), sf, "Deaths")
    # OutMigrations per year
    sf = filter(r -> r.OutMigration == 1, df)
    sf = combine(groupby(sf, [:CalendarYear]), nrow => :n)
    transform!(sf, names(sf, Int8) .=> ByRow(Int), renamecols=false) #needed by XLSX
    sort!(sf)
    addsheet!(joinpath(basedirectory, node, "Episodes", "QC", "EpisodesYrAgeQA.xlsx"), sf, "OutMigrations")
    # LocationExits per year
    sf = filter(r -> r.LocationExit == 1, df)
    sf = combine(groupby(sf, [:CalendarYear]), nrow => :n)
    transform!(sf, names(sf, Int8) .=> ByRow(Int), renamecols=false) #needed by XLSX
    sort!(sf)
    addsheet!(joinpath(basedirectory, node, "Episodes", "QC", "EpisodesYrAgeQA.xlsx"), sf, "LocationExits")
    # ExtResEnds per year
    sf = subset(df, :ExtResEnd => r -> r .== 1)
    sf = combine(groupby(sf, [:CalendarYear]), nrow => :n)
    transform!(sf, names(sf, Int8) .=> ByRow(Int), renamecols=false) #needed by XLSX
    sort!(sf)
    addsheet!(joinpath(basedirectory, node, "Episodes", "QC", "EpisodesYrAgeQA.xlsx"), sf, "ExtResEnds")
    # Refusals per year
    sf = subset(df, :Refusal => r -> r .== 1)
    sf = combine(groupby(sf, [:CalendarYear]), nrow => :n)
    transform!(sf, names(sf, Int8) .=> ByRow(Int), renamecols=false) #needed by XLSX
    sort!(sf)
    addsheet!(joinpath(basedirectory, node, "Episodes", "QC", "EpisodesYrAgeQA.xlsx"), sf, "Refusals")
    # LostToFollowUps per year
    sf = subset(df, :LostToFollowUp => r -> r .== 1)
    sf = combine(groupby(sf, [:CalendarYear]), nrow => :n)
    transform!(sf, names(sf, Int8) .=> ByRow(Int), renamecols=false) #needed by XLSX
    sort!(sf)
    addsheet!(joinpath(basedirectory, node, "Episodes", "QC", "EpisodesYrAgeQA.xlsx"), sf, "LostToFollowUps")
    # Current per year
    sf = subset(df, :Current => r -> r .== 1)
    sf = combine(groupby(sf, [:CalendarYear]), nrow => :n)
    transform!(sf, names(sf, Int8) .=> ByRow(Int), renamecols=false) #needed by XLSX
    sort!(sf)
    addsheet!(joinpath(basedirectory, node, "Episodes", "QC", "EpisodesYrAgeQA.xlsx"), sf, "Current")
    #Current before end
    sf = filter(r -> r.Episode < r.Episodes & r.Current == 1, df)
    transform!(sf, names(sf, Int8) .=> ByRow(Int), renamecols=false) #needed by XLSX
    addsheet!(joinpath(basedirectory, node, "Episodes","QC", "EpisodesYrAgeQA.xlsx"), sf, "CurrentBeforeEnd")
    #Died before end
    sf = filter(r -> r.Episode < r.Episodes & r.Died == 1, df)
    transform!(sf, names(sf, Int8) .=> ByRow(Int), renamecols=false) #needed by XLSX
    addsheet!(joinpath(basedirectory, node, "Episodes","QC", "EpisodesYrAgeQA.xlsx"), sf, "DiedBeforeEnd")
    #Enumeration after episode 1
    sf = filter(r -> r.Episode > 1 & r.Enumeration == 1, df)
    transform!(sf, names(sf, Int8) .=> ByRow(Int), renamecols=false) #needed by XLSX
    transform!(sf, names(sf, Int16) .=> ByRow(Int), renamecols=false) #needed by XLSX
    addsheet!(joinpath(basedirectory, node, "Episodes","QC", "EpisodesYrAgeQA.xlsx"), sf, "EnumerationAfterStart")
    #Died prior to or at end of episode, without Died flags
    sf = filter(r -> !ismissing(r.DoD) && r.DoD <= r.EndDate && r.Died == 0, df)
    transform!(sf, names(sf, Int8) .=> ByRow(Int), renamecols=false) #needed by XLSX
    transform!(sf, names(sf, Int16) .=> ByRow(Int), renamecols=false) #needed by XLSX
    addsheet!(joinpath(basedirectory, node, "Episodes","QC", "EpisodesYrAgeQA.xlsx"), sf, "MissingDiedFlag")
    #Died flag but no DoD
    sf = filter(r -> ismissing(r.DoD) && r.Died == 1, df)
    transform!(sf, names(sf, Int8) .=> ByRow(Int), renamecols=false) #needed by XLSX
    transform!(sf, names(sf, Int16) .=> ByRow(Int), renamecols=false) #needed by XLSX
    addsheet!(joinpath(basedirectory, node, "Episodes","QC", "EpisodesYrAgeQA.xlsx"), sf, "DiedWithoutDoD")
    #Died flag with ExtResEnd
    sf = filter(r -> r.Died == 1 && r.ExtResEnd == 1, df)
    transform!(sf, names(sf, Int8) .=> ByRow(Int), renamecols=false) #needed by XLSX
    transform!(sf, names(sf, Int16) .=> ByRow(Int), renamecols=false) #needed by XLSX
    addsheet!(joinpath(basedirectory, node, "Episodes","QC", "EpisodesYrAgeQA.xlsx"), sf, "DiedFlagWithExtResEnd")
    #Episodes breakdown
    e = freqtable(df, :Episodes)
    addsheet!(joinpath(basedirectory, node, "Episodes","QC", "EpisodesYrAgeQA.xlsx"), e, "EpisodesFreq")
    return nothing
end
function arrowtostata(node, inputfile, outputfile)
    basedirectory = settings.BaseDirectory
    df = Arrow.Table(joinpath(basedirectory, node, "Episodes", inputfile * ".arrow")) |> DataFrame
    arrowfile = joinpath(basedirectory, node, "Episodes", outputfile * ".arrow")
    Arrow.write(arrowfile, df, compress = :zstd)
    statafile = joinpath(basedirectory, node, "Episodes", outputfile * ".dta")
    R"""
    library(arrow)
    library(rio)
